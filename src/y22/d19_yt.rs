/* results
    ...
    thonk max_result
    part 1 result: 994 in 3.358676417s
    memo refs: 82929763
    memo hits: 37816021 (46%)
    states visited: 82929763
    states skipped: 0

    thonk enough bots
    part 1 result: 994 in 417.49375ms
    memo refs: 12741390
    memo hits: 4955799 (39%)
    states visited: 12741390
    states skipped: 0

    thonk don't idle
    part 1 result: 994 in 23.98975ms
    memo refs: 589132
    memo hits: 26567 (5%)
    states visited: 589132
    states skipped: 0

    thonk geodes first
    part 1 result: 994 in 23.680459ms
    memo refs: 566020
    memo hits: 25144 (4%)
    states visited: 566020
    states skipped: 0
    ...
    thonk no memo
    part 1 result: 994 in 2.733042ms
    memo refs: 0
    memo hits: 0 (NaN%)
    states visited: 674356
    states skipped: 0
...
use std::{cell::UnsafeCell, hash::Hasher};
...
mod baseline {
    ...
    pub fn part_1<F: Fn(&Blueprint, u8) -> u32>(bps: &[Blueprint], f: F) {
        ...
        for bp in bps {
            let t0 = std::time::Instant::now();
            let geodes = f(bp, 24);
            println!("geodes: {} in {:?}", geodes, t0.elapsed());
...
fn hash_bench_thing() {
    ...
    impl core::hash::Hash for Pack64 {
        fn hash<H: Hasher>(&self, state: &mut H) {
            state.write_u64(unsafe { core::mem::transmute(*self) });
        }
    }

    #[inline]
    fn go<T: core::hash::Hash>(v: T) -> (u64, std::time::Duration) {
        let t0 = std::time::Instant::now();
        ...
        (hasher.finish(), t0.elapsed())
    }

    go(Pack32::default());

    let (h32, t32) = go(Pack32::default());
    let (h8,  t8)  = go(Pack8::default());
    let (h64, t64) = go(Pack64::default());

    println!("Pack32: hash {} in {:?}", h32, t32);
    println!("Pack8:  hash {} in {:?}", h8,  t8);
    println!("Pack64: hash {} in {:?}", h64, t64);
}
